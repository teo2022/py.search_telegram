...
def find_last_parsed_date(path):  # определение даты, с которой начинать парсинг
    ...
    oldest = datetime.datetime.strptime("2023-01-01 00:00:00+00:00", "%Y-%m-%d %H:%M:%S%z")
    temp = oldest
...
def parse(client, url):  # сбор сообщений из канала
    ...
    for message in client.iter_messages(url, reverse=True, offset_date=oldest):  # итератор по сообщениям (урл - ссылка
        ...
        try:
            ...
            if message.message:
                i += 1
                data.append({"id":directory_name, "date":str(message.date), "url": url + '/' + str(message.id), "text":clearify_text(msg=message)})
                if i == 100:
                    i = 0
                    with open(f"pars/{channel_id}/data.json", 'w') as f:
